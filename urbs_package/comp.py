...
def compare_scenarios(result_files, output_filename):
    ...
    # REPORT
    def report(costs, esums):
        with pd.ExcelWriter('{}.{}'.format(output_filename, 'xlsx')) as writer:
            costs.to_excel(writer, 'Costs')
            esums.to_excel(writer, 'Energy sums')

    scenario_names = derive_scenarios()
    costs, esums = read_files(scenario_names)
    costs, esums = analyse(costs, esums)
    costs, esums = plot(costs, esums)
    report(costs, esums)
